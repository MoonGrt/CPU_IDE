...
class IDE(QMainWindow):
    ...
    def assemble(self):
        ...
        try:
            ...
            self.message_showmessage("Please input file\n")
            return

        if not program_code:
            self.message_showmessage("Empty file\n")
            return

        variable_list = []
        try:
            # 编译
            assembly_code = self.Compiler.compiler("main", parser().parse(lexer().lexe(program_code)), variable_list)
            assembly_code = self.Compiler.hide_label(assembly_code)
            ...
            self.Simulator = simulator(self.Assembler.getlabel().get('main'))

            self.assemble_code_area.setPlainText(assembly_code.expandtabs(4))
            self.machine_code_area.setPlainText(machine_code)

            # 按行分割并转换为整数列表
            lines = machine_code.strip().split('\n')
            # 将二进制字符串转换为整数
            self.Simulator.inst_mem = [int(line, 2) for line in lines]

            # 一旦执行了编译，就启用其他操作
            self.run_Action.setEnabled(True)
            self.run_step_Action.setEnabled(True)
            self.run_undo_Action.setEnabled(True)
            self.reset_Action.setEnabled(True)
            self.stop_Action.setEnabled(True)
            if self.Downloader.serial_com.is_open:
                self.download_Action.setEnabled(True)
        except:
            self.message_showmessage("Compile error\n")
            return
        
        # 向表格填入数据
        self.fillCode(assembly_code, machine_code, self.Assembler.get_codebasic())
        self.fillLabel(self.Assembler.getlabel())
